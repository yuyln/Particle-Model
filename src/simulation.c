...
f64 system_energy(BoxedParticles bp, Table potential, DefectMap defect_map) {
    ...
    for (u64 ip = 0; ip < bp.ps.len; ++ip) {
        Particle p = bp.ps.items[ip];
        s64 idx = boxed_particles_get_idx(bp, bp.ps.items[ip].pos.x);
        s64 idy = boxed_particles_get_idx(bp, bp.ps.items[ip].pos.y);
        for (s32 dy = -1; dy <= 1; ++dy) {
            for (s32 dx = -1; dx <= 1; ++dx) {
                ...
                for (u64 n = 0; n < bp.n_particle[i_interaction]; ++n)
                    if (!CLOSE_ENOUGH(p.pos.x, bp.ps.items[indices_current[n]].pos.x, EPS) &&
                        !CLOSE_ENOUGH(p.pos.y, bp.ps.items[indices_current[n]].pos.y, EPS))
                        energy += 0.5 * particle_energy_point(bp.ps.items[indices_current[n]], p.pos, potential);
            }
        }
        energy += defect_map_potential_xy(p.pos.x, p.pos.y, defect_map);
    }
    return energy;
}
