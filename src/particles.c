...
f64 particle_energy_point(Particle p, v2d point, const Table potential) {
    ...
    return p.u0 * table_get_value(potential, sqrt(distance_squared / p.radius));
}
...
v2d particle_force_point(Particle p, v2d point, const Table potential) {
    ...
    return v2d_fac(distance_v2d, -p.u0 * table_get_derivative(potential, distance / p.radius) * 1.0 / p.radius);
}
